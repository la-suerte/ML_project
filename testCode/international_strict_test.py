...
# ============================================================================
# CONFIGURATION (MODE STRICT - MODIFIÉ PAR SACHA)
# ============================================================================

# On garde les mêmes dates
START_DATE = datetime(2015, 10, 19)
END_DATE = datetime(2025, 10, 16)

# TA "FEATURE" : On exige une qualité de données irréprochable
COMPLETENESS_THRESHOLD = 0.95  # On passe de 80% à 95% (Qualité Max)
INDEX_THRESHOLD = 0.80         # L'indice doit être représentatif

# On teste un modèle plus simple (Rang 5 au lieu de 7) pour voir si c'est plus robuste
CONFIGS = [
    {
        'name': 'Strict_Quality_ShortTerm',
        'svd_rank': 5,              # Rang 5 (Moins de bruit que 7)
        'train_years': 2,
        'test_months': 12,          # Test sur 1 an
        'max_position': 0.08,       # Max 8%
        'short_allowed': False
    },
    {
        'name': 'Strict_Quality_LongTerm',
        'svd_rank': 5,              # Rang 5
        'train_years': 4,           # On entraîne sur 4 ans (plus long)
        'test_months': 48,          # Test sur 4 ans
        'max_position': 0.08,
        'short_allowed': False
    }
]
